 ...
 *
 * MediaTek dp intf driver header
 *
 * Copyright (c) 2023 MediaTek Inc.
 * Author: Tommy Chen <tommyyl.chen@mediatek.com>
 */

#ifndef __DP_INTF_H__
#define __DP_INTF_H__
...
#define OUT_BIT_MASK			(0x3 << 16)
#define OUT_BIT_8			(0x00 << 16)
#define OUT_BIT_10			(0x01 << 16)
#define OUT_BIT_12			(0x02 << 16)
#define OUT_BIT_16			(0x03 << 16)
...
#define DP_INTF_REG_READ(addr) readl(addr)
#define DP_INTF_REG_WRITE(addr, val) writel(val, addr)

struct display_mode {
	int clock;		/* Pixel clock in kHz in kHz */
	int hdisplay;
	int hsync_start;
	int hsync_end;
	int htotal;
