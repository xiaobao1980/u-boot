...
void mtk_dp_intf_vclk(u32 pixel_clk_freq)
{
	...
	DP_CLOCK_REG_WRITE(TVDPLL_CON4, DP_CLOCK_REG_READ(TVDPLL_CON4) | RG_TVDPLL1_SDM_PWR_ON);
	udelay(100);
	DP_CLOCK_REG_WRITE(TVDPLL_CON4, DP_CLOCK_REG_READ(TVDPLL_CON4) &
			   (~(RG_TVDPLL1_SDM_ISO_EN)));
	udelay(100);

	DP_CLOCK_REG_WRITE(TVDPLL_CON2, div << RG_TVDPLL1_POSDIV_SHIFT |
		(pcw & RG_TVDPLL1_SDM_PCW_MASK));

	val |= RG_TVDPLL1_EN;
	DP_CLOCK_REG_WRITE(TVDPLL_CON0, val);

	DP_CLOCK_REG_WRITE(CLK_CFG_10_CLR, CLK_HF_FEDP_CK_SEL_MASK);
	DP_CLOCK_REG_WRITE(CLK_CFG_10_SET,
			   (clksrc << CLK_HF_FEDP_CK_SEL_SHIFT) & CLK_HF_FEDP_CK_SEL_MASK);
	DP_CLOCK_REG_WRITE(CLK_CFG_10_CLR, PDN_HF_FEDP_CK);
